...
struct point {
	...
	point rot90()  const { return point(-y,  x); } // `counter-clockwise`
	...
	double distLP(const point &p1, const point &p2) const { // `dist from *this to line p1->p2`
		const point &q = *this; return fabs(det(p2 - p1, q - p1)) / (p2 - p1).len();
	}
...
struct circle {
	...
	circle() : o(), r(0.0), rSqure(0.0) {}
	circle(const point &o, const double &r) : o(o), r(r), rSqure(r * r) {}
	bool inside(const point &a) { return (a - o).len() < r + EPS; } // `非严格`
